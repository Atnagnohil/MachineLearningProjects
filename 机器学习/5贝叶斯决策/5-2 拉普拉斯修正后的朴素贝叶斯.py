...
def trainModel(trainset, trainlabels):
    ...
    # print(labels)
    for label in labels:
        prior_probability[label] = len(trainlabels[trainlabels==label]) + 1
    ...
    for key in condition_probability_cnt.keys():
        Ni = len(set(trainset[:, key[1]]))
        condition_probability[key] = (condition_probability_cnt[key] + 1)/ (prior_probability[key[0]]+Ni)
    for label in labels:
        prior_probability[label] = prior_probability[label] / (n + len(labels))
